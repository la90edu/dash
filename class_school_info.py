...
#         return float(anigmas.future_future_result(self.df))
  ...
  def get_second_biggest_delta_from_global(self):
        delta_dict=self.return_delta_from_global_as_dict()
        max_key_first  = max(delta_dict, key=lambda k: delta_dict[k])#abs(delta_dict[k]))
        delta_dict.pop(max_key_first)
        max_key_second = max(delta_dict, key=lambda k: delta_dict[k])#abs(delta_dict[k]))
        return max_key_second
  ...
  def get_first_high_hegid_to_fix(self,anigma_name):
        return anigmas.get_max_average_higed_from_anigma(self.df,self.return_biggest_delata_from_global())
    
#get seconrd high heged
# get first low heged
# get second low heged
