...
def get_max_average_higed_from_anigma(df,anigma_name):
    ...
    st.write(f"Column with maximum average is: {max_col}")
    st.write(f"chosen anigma:{anigma_name}")
    st.write(result)
    st.dataframe(df)
    return max_col

def get_min_average_higed_from_anigma(df,anigma_name):
    df_cuted=cut_df(df,anigma_name)
    result=actions.return_mean_dict(df_cuted)
    min_col = min(result, key=result.get)
    st.write(f"Column with minimum average is: {min_col}")
    st.write(f"chosen anigma:{anigma_name}")
    st.write(result)
    st.dataframe(df)
    return min_col
